<template>
  <NuxtLink
    ...
    class="wrapper-card-index relative bg-white rounded-lg shadow-sm border border-gray-100 hover:shadow-md transition-shadow overflow-hidden flex h-full min-h-[200px] flex-col sm:flex-row"
  >
    <div
    ...
    <div class="flex flex-col sm:flex-row gap-4 p-4 w-full">
      <!-- Left Content -->
      <div class="sm:w-1/2 flex flex-col justify-between gap-3">
        <div class="space-y-2">
          <h3
        ...
        <!-- Add Button -->
        <Button class="flex gap-2 w-full sm:w-auto" @click.prevent="add(product)">
          <Icon icon="mdi:cart-plus" class="w-4 h-4" />
          <span class="text-xs font-bold">Thêm giỏ hàng</span>
        </Button>
      </div>
      <!-- Right Image -->
      <div class="sm:w-1/2 relative overflow-hidden rounded-lg h-44 sm:h-auto">
        <img
